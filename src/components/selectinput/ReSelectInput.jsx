...
import ValidationComponent from "robe-react-ui/lib/validation/ValidationComponent";
 ...
 */
export default class ReSelectInput extends ValidationComponent {
    ...
    __renderSingleSelect() {
        ...
        for (let i = 0; i < items.length; i++) {
            let item = items[i];
            let index = Arrays.indexOf(this.props.disabledValues, this.props.valueField, item);
        ...
        return (
            <FormControl
                ...
                value={this.props.value ? this.props.value[this.props.valueField] : ""}
    ...
    __renderMultiValues(values) {
        ...
        for (let i in values) {
            let value = values[i];
            let item = this.getValueByKey(this.props.items, this.props.valueField, value);
            if (item !== undefined) {
                valueList.push(
    ...
    getValueByKey(source, key, target) {
    ...
    __onSelectMultiItem(e) {
        ...
        let itemValue = this.props.items[0][this.props.valueField];
        let newValue;
        if (typeof itemValue === "number")
            newValue = parseInt(key);
        else if (typeof itemValue === "string")
            newValue = key.toString();
        let item = Arrays.getValueByKey(this.props.items, this.props.valueField, newValue);
        if (Arrays.indexOfByKey(value, this.props.valueField, newValue) === -1) {
            value.push(item);
        }
        this.__callOnChange(value, this.state.value);
    }
    ...
    __onSelectSingleItem(e) {
        ...
        let item = Arrays.getValueByKey(this.props.items, this.props.valueField, newValue);
        this.__callOnChange(item, this.state.value);
    }
