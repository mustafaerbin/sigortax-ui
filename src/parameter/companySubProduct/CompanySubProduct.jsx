...
import CompanySubProductModel from "./CompanySubProductModel.json";
...
export default class CompanySubProduct extends ShallowComponent {
    ...
    companyProductRequest = new AjaxRequest({
        url: "company-product",
        type: "GET"
    });
    ...
    constructor(props) {
        ...
        let store = new Store({
            ...
            idField: CompanySubProduct.idField,
            autoLoad: true
        });

        this.state = {
            fields: CompanySubProductModel.fields,
            store: store,
            showModal: false,
            item: {},
            items: [],
    ...
    __cellRenderer(idx: number, fields: Array, row: Object) {
        ...
        if (fields[idx].name == 'companyOid') {
            return <td key={fields[idx].name}>{row.companyProduct.company.label}</td>;
        }
        if (fields[idx].name == 'companyProductOid') {
            return <td key={fields[idx].name}>{row.companyProduct.label}</td>;
        }
